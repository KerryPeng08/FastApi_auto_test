...
async def download_project_name(project_id: int, db: Session = Depends(get_db)):
    ...
    if pro_info:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='未查询到项目数据')
...
@pool.get('/getApi/{api_id}', response_model=schemas.YApiDataOut, name='获取单个接口的信息')
async def get_api_id(api_id: int, db: Session = Depends(get_db)):
    return await pool_crud.get_api(db=db, api_id=api_id)


@pool.get('/getApi/{title}', response_model=schemas.YApiDataOut, name='获取单个接口的信息')
async def get_api_name(title: str, db: Session = Depends(get_db)):
    return await pool_crud.get_api(db=db, title=title)
